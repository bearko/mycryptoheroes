...
- `Data/Characters/characters.json`: オリジナルキャラクター（クリスくん/マインちゃん/マイクリ）の立ち絵・差分 59 コマの一覧。
...
- `Image/Characters/Gif/*.gif`: 立ち絵のアニメーションGIF。
...
`Image/Characters` のクリスくん/マインちゃん/マイクリは、マイクリの派生コンテンツや解説動画で使えるオリジナルキャラクターです。マイクリ公式が提供する「マイクリ画像」ではないため、公式デザインガイドラインの制約対象ではありません。
...
- マイクリ（ID: `maycri` / 英字表記: Maycri）: メガホンを持った丸いマスコットキャラクター。キャンバスサイズは 64x64px。名称はソースファイル名由来の暫定表記です。
...
| `Image/Characters/Gif/*.gif` | アニメーションGIF。そのまま再生できる形式が必要な場合はこちらを利用します。 |
...
| `maycri` | マイクリ | 7 | 待機アニメーション（目の表情差分） |
...
- `Image/Characters` のオリジナルキャラクター（クリスくん/マインちゃん/マイクリ）は運営側が提供する素材ではないため、このガイドラインの対象外です。利用条件は「オリジナルキャラクター補足」を参照してください。
- 上記以外の画像は使用できません。
